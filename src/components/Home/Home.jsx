...
export default function Home() {
  ...
  const [firstPRoductIndex, setFirstProductIndex] = useState(
    Math.trunc(Math.random() * data.length)
  );
  ...
  console.log(firstPRoductIndex, secondPRoductIndex, thirdPRoductIndex);

  useEffect(() => {
    if (firstPRoductIndex == secondPRoductIndex) {
      setSecondProductIndex(Math.trunc(Math.random() * data.length));
    }
    if (
      thirdPRoductIndex == secondPRoductIndex ||
      thirdPRoductIndex == firstPRoductIndex
    ) {
      setThirdProductIndex(Math.trunc(Math.random() * data.length));
    }
  }, []);
  let presentation = [
    data[firstPRoductIndex],
    data[secondPRoductIndex],
    data[thirdPRoductIndex],
  ];
  return (
    <div className="home">
      <div className="top">
          ...
          {data.map((elem, i) => (
            ...
            >
              ...
              <div className="product-info" style={{}}>
                <p>{elem.name}</p>{" "}
                <p>
                  <span style={{ color: "#AD7A5F" }}>${elem.price}</span>
                </p>
              </div>
            </div>
          ))}
